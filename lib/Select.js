  ...
  * */
   ...
   */
	...
	componentDidUpdate: function componentDidUpdate() {
		...
		if (this._focusedOptionReveal) {
			if (this.refs.focused && this.refs.menu) {
				var focusedDOM = this.refs.focused; // div
				var menuDOM = this.refs.menu; // div
	...
	setValue: function setValue(value) {
		// Disable the auto-focus, because we do not want it to happen.
		...
		setTimeout((function() {
			...
			// console.log('Select.setValue(): [setTimeout] input=',input);
			if(input) {
				var elem = input.getInput(); // Input with our options
				// console.log('Select.setValue(): [setTimeout] elem=',elem);
				if(elem) {
					// console.log('Select.setValue(): [setTimeout] blur()');
					elem.blur();
				}
			}
		}).bind(this), 250);
	...
	handleInputFocus: function handleInputFocus(event) {
		this.setState({
			...
			filteredOptions: this.filterOptions(this.props.options)
		});
	...
	handleInputBlur: function handleInputBlur(event) {
		...
		if(this.state.inputValue) {
			// onBlur we perform the same behaviour as when pressing enter.
			// console.log('Select.handleInputBlur(): [select] state.inputValue= ('+typeof this.state.inputValue+') "' + this.state.inputValue + '"');
			this.selectFocusedOption();
		} else {
			// reset when they left the input empty.
			// console.log('Select.handleInputBlur(): [reset] state.value= ('+typeof this.state.value+') "' + this.state.value + '"');
			this.resetValue();
		}
		// Disable the auto-focus [in setValue()] otherwise this will be focussed again and we cannot leave it.
		this._focusAfterUpdate = false;
	...
	shouldComponentUpdate: function(nextProps, nextState) {
		...
		if(this.state.inputValue !== nextState.inputValue) {
			...
			if(this.props.onChangeInput) {
				setTimeout(function() {
					self.props.onChangeInput(self.state);
				},0);
			}
		}
		return true;
	...
	filterOptions: function filterOptions(options, values) {
		...
		if (this.props.filterOptions) {
			...
			var filterOption = function filterOption(op) {
				...
				return !filterValue || this.props.matchPos === "start" ? this.props.matchProp !== "label" && valueTest.toLowerCase().substr(0, filterValue.length) === filterValue.toLowerCase() || this.props.matchProp !== "value" && labelTest.toLowerCase().substr(0, filterValue.length) === filterValue.toLowerCase() : this.props.matchProp !== "label" && valueTest.toLowerCase().indexOf(filterValue.toLowerCase()) >= 0 || this.props.matchProp !== "value" && labelTest.toLowerCase().indexOf(filterValue.toLowerCase()) >= 0;
			};
	...
	render: function render() {
		...
		if (this.props.disabled || !this.state.isFocused && !this.state.inputValue && (!this.props.multi || !value.length)) {
